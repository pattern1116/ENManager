...
2. **Cloudflare Tunnel**에 Public Hostname 추가 → `en-manager.matildabc.com` → `http://localhost:<포트>`.
3. **Cloudflare Access**(이메일 인증)로 보호 — 안 그러면 누구나 이 맥의 로컬 모델(Gemma/STT)을 굴릴 수 있다.
